...
// Worker polls the outbox table for PENDING notification events and publishes
// them to the user.notifications topic exchange in RabbitMQ.
type Worker struct {
...
// SetupExchange declares the topic exchange and binds queues so the
// notification-service consumers receive events by routing key.
